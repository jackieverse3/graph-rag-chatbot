...
@app.get("/stats")
def get_stats():
    """Queries Neo4j to count current nodes and relationships."""
    driver = GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASSWORD))
    try:
...
def build_graph_text(payload: TextInput):
    ...
    raise HTTPException(status_code=500, detail="Graph build failed from pasted text.")


@app.post("/build-graph/pdf")
async def build_graph_pdf(file: UploadFile = File(...)):
